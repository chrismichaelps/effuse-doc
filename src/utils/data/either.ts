const TypeId = Symbol.for('app/Either');
...
export function match<E, A, B, C>(
	...
	return isLeft(either)
		? cases.onLeft(either.left)
		: cases.onRight(either.right);
}
...
export function mapLeft<E, A, E2>(
	...
	if (f === undefined) {
		...
		return <A2>(either: Either<E, A2>) =>
			isRight(either) ? either : left(fn(either.left));
	}
	const either = eitherOrF as Either<E, A>;
...
export function flatMap<E, A, E2, B>(
	...
	if (f === undefined) {
		...
		return <E3>(either: Either<E3, A>) =>
			isLeft(either) ? either : fn(either.right);
	}
	const either = eitherOrF as Either<E, A>;
	return isLeft(either) ? either : f(either.right);
}

export const getOrElse = <E, A, B>(
	either: Either<E, A>,
	orElse: (e: E) => B
): A | B => (isLeft(either) ? orElse(either.left) : either.right);
