...
| `layers`      | 宣言したレイヤーの props とサービスへの型付きアクセス |
| `scope`       | クリーンアップとファイナライザーを管理                |

## 組み込みユーティリティフック

Effuse は一般的なパターンのために `script` コンテキスト内にいくつかの組み込みフックを提供します：

### `useCallback(fn, deps?)`

レンダリング間で安定したアイデンティティを維持するために関数をメモ化します。
...
計算値をメモ化します。それ自体がリアクティブシグナルである必要はないがキャッシュされるべき高価な計算に役立ちます。
