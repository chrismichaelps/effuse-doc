...
effect(
  () => {
    console.log('エフェクトを実行中');
  },
  {
    debounce: 300, // 実行をデバウンス
    timeout: 5000, // 非同期実行のタイムアウト
    retry: 3, // エラー時の再試行回数
...
| オプション | 型                          | 説明                                                 |
| :--------- | :-------------------------- | :--------------------------------------------------- |
| `debounce` | `number`                    | 遅延後に実行を延期                                   |
| `timeout`  | `number`                    | 実行に許可される最大時間                             |
| `retry`    | `number \| RetryOptions`    | 失敗したエフェクトを再試行する戦略                   |
| `flush`    | `'pre' \| 'post' \| 'sync'` | レンダリングに関連してエフェクトを実行するタイミング |

### 複数のシグナルを監視

複数のシグナルを同時に監視するには `watchMultiple` を使用します：
